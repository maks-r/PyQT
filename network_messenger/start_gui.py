...
class Launcher:
    ...
    def __init__(self, master):
        self.master = master
        master.title('Запуск консольного мессенджера')
    ...
    def start(self):
        self.processes.append(subprocess.Popen(f'python run_server.py -p {self.port.get()} -a {self.host.get()}',
                                               creationflags=subprocess.CREATE_NEW_CONSOLE))

        for i in range(int(self.clients.get())):
            self.processes.append(subprocess.Popen(f'python run_client.py {self.host.get()} {self.port.get()} '
                                                   f'-n test{i + 1}', creationflags=subprocess.CREATE_NEW_CONSOLE))
