...
def gui_create_model(database):
    ...
    list_table.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт', 'Время подключения', 'Время отключения'])
    for row in list_users:
        user, ip, port, time, end_time = row
        ...
        end_time = QStandardItem(str(time.replace(microsecond=0)))
        end_time.setEditable(False)
        list_table.appendRow([user, ip, port, time, end_time])
    return list_table
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        # Кнопка выхода
        self.exitAction = QAction(QIcon('img/exit.jpg'), 'Выход', self)
        self.exitAction.setShortcut('Ctrl+Q')
        self.exitAction.triggered.connect(qApp.quit)

        # Кнопка обновить список клиентов
        self.refresh_button = QAction(QIcon('img/replace.jpg'), 'Обновить список', self)

        # Кнопка вывести историю сообщений
        self.show_history_button = QAction(QIcon('img/history_icon.jpg'), 'История клиентов', self)
        ...
        self.setWindowTitle('Server main')
...
if __name__ == '__main__':
    ...
    test_list.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт', 'Время подключения', 'Время отключения'])
    test_list.appendRow(
        [QStandardItem('test1'), QStandardItem('192.198.0.5'), QStandardItem('23544'), QStandardItem('16:20:34'), QStandardItem('16:20:34')])
    test_list.appendRow(
        [QStandardItem('test2'), QStandardItem('192.198.0.8'), QStandardItem('33245'), QStandardItem('16:22:11'), QStandardItem('16:20:34')])
